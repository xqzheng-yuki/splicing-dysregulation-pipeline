...
directory = "/mnt/gtklab01/xiaoqing/result_for_thesis/origin+mashmap"
...
for sample in samples:
    ...
    try:
        ...
        with open(os.path.join(filt_dir, "aux_info", "meta_info.json")) as f:
            meta_filt = json.load(f)

        # TPM tables
        df_full = pd.read_csv(os.path.join(full_dir, "quant.sf"), sep="\t")
        df_filt = pd.read_csv(os.path.join(filt_dir, "quant.sf"), sep="\t")
        ...
        # Correlation
        corr, _ = pearsonr(np.log2(df_merged["TPM_full"] + 1e-6), np.log2(df_merged["TPM_filtered"]+ 1e-6))

        # Save plot
        plt.figure(figsize=(6,6))
        sns.scatterplot(data=df_merged, x="TPM_full", y="TPM_filtered", alpha=0.5)
        plt.xscale("log")
        plt.yscale("log")
        plt.plot([1e-3, 1e5], [1e-3, 1e5], 'r--')
        plt.xlabel("TPM (Original Decoy)")
        plt.ylabel("TPM (Filtered Decoy)")
        ...
        summary_rows.append({
            ...
            "num_processed_filt": meta_filt.get("num_processed", 0),
            "num_mapped_filt": meta_filt.get("num_mapped", 0),
            "num_decoy_fragments_filt": meta_filt.get("num_decoy_fragments", 0),
            "percent_mapped_filt": meta_filt.get("percent_mapped", 0),
...
for i, (sample, df_merged, corr) in enumerate(plots_data):
    ax = axes[i]
    sns.scatterplot(data=df_merged, x="TPM_full", y="TPM_filtered", alpha=0.5, ax=ax)
    ax.set_xscale("log")
    ax.set_yscale("log")
    ax.plot([1e-3, 1e5], [1e-3, 1e5], 'r--')
    ax.set_title(f"{sample}\nr = {corr:.2f}")
    ax.set_xlabel("TPM (Original Decoy)")
    ax.set_ylabel("TPM (Filtered Decoy)")

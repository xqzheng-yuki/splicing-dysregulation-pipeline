...
directory = "/mnt/gtklab01/xiaoqing/result_for_thesis/mashmap+decap"
...
for sample in samples:
    ...
    try:
        ...
        df_merged = df_filt[["Name", "TPM"]].merge(
            df_decap[["Name", "TPM"]],
            on="Name",
            suffixes=("_filtered", "_decap")
        )
        ...
        sns.scatterplot(data=df_merged, x="TPM_filtered", y="TPM_decap", alpha=0.5)
        plt.xscale("log")
        plt.yscale("log")
        plt.plot([1e-3, 1e5], [1e-3, 1e5], 'r--')
        plt.xlabel("TPM (Filtered Decoy)")
        ...
        summary_rows.append({
            "Sample": sample,
            "num_processed_filtered": meta_filt.get("num_processed", 0),
            "num_mapped_filtered": meta_filt.get("num_mapped", 0),
            "num_decoy_fragments_filtered": meta_filt.get("num_decoy_fragments", 0),
            "percent_mapped_filtered": meta_filt.get("percent_mapped", 0),
...
for i, (sample, df_merged, corr) in enumerate(plots_data):
    ax = axes[i]
    sns.scatterplot(data=df_merged, x="TPM_filtered", y="TPM_decap", alpha=0.5, ax=ax)
    ax.set_xscale("log")
    ax.set_yscale("log")
    ax.plot([1e-3, 1e5], [1e-3, 1e5], 'r--')
    ax.set_title(f"{sample}\nr = {corr:.2f}")
    ax.set_xlabel("TPM (Filtered Decoy)")

...
bw_files <- list.files(path = ".", pattern = "^unmapped_CTX_[0-9]{3}_(d|m1|m2|u).bw$", full.names = TRUE)\
...
treatment_bw <- map(bw_files[tags %in% treatment_group], ~ {
  bw_file <- import_bigwig(.x)
  bw_file
})
names(treatment_bw) <- basename(bw_files[tags %in% treatment_group])
control_bw <- map(bw_files[tags %in% control_group], ~ {
  bw_file <- import_bigwig(.x)
  bw_file
})
names(control_bw) <- basename(bw_files[tags %in% control_group])

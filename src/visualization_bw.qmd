...
get_track_filter <- function(track_list, group_tags, condition_tag = NULL) {
  ...
  ### try this out for ReferenceDataTrack
  for (i in seq_along(filter_tracks)) {
    ...
    filter_tracks[[i]] <- DataTrack(
      range = track_list[[track_name]]@stream(file),
      genome = "mm39",
      type = "l",
...
control_shades <- setNames(control_shades, condition_tag)
treatment_shades <- generate_shades("#912C2C")
treatment_shades <- setNames(treatment_shades, condition_tag)
# color_list <- c(control_shades,treatment_shades)
...
plotTracks(list(Trail_track,bw_track_list[["CTX_104_d"]],Trail_track_raw),from=start(gr),to=end(gr),chromosome = as.character(chrom(gr)))
```

...
while read SAMPLE_NAME; do
    ...
    do
        ...
        seqtk subseq $RNA_READS_DIR/${SAMPLE_NAME}_1.fq.gz $file > $UNMAPPED_SEQ_DIR/${SAMPLE_NAME}_${type}_unmapped_seq_1.fq
        seqtk subseq $RNA_READS_DIR/${SAMPLE_NAME}_2.fq.gz $file > $UNMAPPED_SEQ_DIR/${SAMPLE_NAME}_${type}_unmapped_seq_2.fq
    done
    index=$(expr $index + 1)
    sleep 1
done < $READS_FILE
